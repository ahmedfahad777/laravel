        ...
        @foreach($sents as $key => $value)
            <tr class="unread" data-messageid="{{$value->status}}">
                ...
                <td class="view-message hidden-xs">
                    <a href="{{ URL::to('inboxes/sent/' . $value->status) }}">{{$value->subject}}</a>
                </td>
                <td class="view-message">
                    <a href="{{ URL::to('inboxes/sent/' . $value->status) }}">{{substr($value->body_description,0,50)}}</a>
                </td>
                <td class="view-message inbox-small-cells">
                </td>
                <td class="view-message text-right">
                    <a href="{{ URL::to('inboxes/sent/' . $value->status) }}">{{$value->created_at}}</a>
                </td>
            </tr>
        @endforeach
