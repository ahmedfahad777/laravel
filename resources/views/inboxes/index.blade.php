                    ...
                    <ul class="dropdown-menu">
                        ...
                        <li>
                            <a href="javascript:;">
                                ...
                                {!! Form::submit('Delete', array('class' => 'btn btn-danger','name' => 'trash')) !!}
                            </a>
                        </li>
                    </ul>
                </div>
        ...
        @foreach($inboxes as $key => $value)
            <tr class="unread" data-messageid="{{$value->id}}">
                ...
                <td class="view-message hidden-xs">
                    <a href="{{ URL::to('inboxes/' . $value->id) }}">{{$value->subject}}</a>
                </td>
                <td class="view-message">
                    <a href="{{ URL::to('inboxes/' . $value->id) }}">{{substr($value->body_description,0,50)}}</a>
                </td>
                <td class="view-message inbox-small-cells">
                </td>
                <td class="view-message text-right">
                    <a href="{{ URL::to('inboxes/' . $value->id) }}">{{$value->created_at}}</a>
                </td>
            </tr>
        @endforeach
